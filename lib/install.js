...
function install (args, cb) {
  ...
  var steps = []
  steps.push(
    [loadCurrentTree, where, installState],
...
function loadCurrentTree (where, installState, cb) {
  chain([
    [readLocalPackageData, where, installState],
...
function loadAllDepsIntoIdealTree (args, installState, cb) {
  ...
  steps.push( args.length
    ? [loadRequestedDeps, args, idealTree, saveDeps, log.newGroup("loadRequestedDeps", 2)]
    : [loadDeps, idealTree, log.newGroup("loadDeps")])
  if (dev) steps.push(
    [loadDevDeps, idealTree, log.newGroup("loadDevDeps", 5)])
  chain(steps, cb)
}
...
function generateActionsToTake (installState, cb) {
  ...
  chain([
    ...
    [diffTrees, currentTree, idealTree, installState.differences, log.newGroup("diffTrees")],
    [decomposeActions, installState.differences, todo, log.newGroup("decomposeActions")]
  ], cb)
}
...
function executeActions (where, installState, cb) {
  ...
  var steps = []
  var trackLifecycle = log.newGroup("lifecycle")
  ...
  steps.push(
    ...
    [doParallel, "remove", staging, todo, log.newGroup("remove")],
    [doSerial,   "finalize", staging, todo, log.newGroup("finalize")],
...
function runTopLevelLifecycles (args, installState, cb) {
  ...
  var trackLifecycle = log.newGroup("top")
...
function readLocalPackageData (where, installState, cb) {
  // TODO: Update readPackageTree to ignore all .dirs in node_modules
  // Also to do what normalizeTree does
...
function loadShrinkwrap (installState, cb) {
  var idealTree = installState.idealTree
  if (!idealTree.package.shrinkwrap) return cb()
    inflateShrinkwrap(idealTree, idealTree.package.shrinkwrap.dependencies, cb)
}
